...
class ImportUrbaweb(BaseImport):
    ...
    def __init__(self, config_file, view, views, limit=None, range=None, licence_id=None, id=None, ignore_cache=False, pg_ignore_cache=False, benchmarking=False, noop=False, iterate=False):
        ...
        engine_cadastral = create_engine('postgresql://{user}:{password}@{host}:{port}/{schema}'.format(
        ...
        self.CODT_DATE = datetime.strptime("01/06/17", "%d/%m/%y")
        ...
        self.allowed_refs = [ref.replace('\n', '') for ref in open('missing_refs.txt', 'r').readlines()]
    ...
    def extract_data(self):
        ...
        for extract_data_view in extract_data_views:
            ...
            for id, licence in folders.iterrows():
                if licence.NUM_PERMIS not in self.allowed_refs:
                    continue
    ...
    def get_portal_type(self, licence):
        ...
        if portal_type in ['BuildLicence', 'Article127', 'UniqueLicence', 'IntegratedLicence']:
            if licence.AUTORISATION and datetime.strptime(licence.AUTORISATION, "%m/%d/%y 00:00:00") >= self.CODT_DATE and int(licence.AUTORISATION[6:8]) < 23:
                print("{} => CODT: {}".format(licence.NUM_PERMIS or licence.N1, licence.AUTORISATION))
                portal_type = "{}{}".format("CODT_", portal_type)
            elif licence.REFUS and datetime.strptime(licence.REFUS, "%m/%d/%y 00:00:00") >= self.CODT_DATE and int(licence.REFUS[6:8]) < 23:
                print("{} => CODT: {}".format(licence.NUM_PERMIS or licence.N1, licence.REFUS))
                portal_type = "{}{}".format("CODT_", portal_type)
        return portal_type
