...
class ImportUrbaweb(BaseImport):
    ...
    def get_portal_type(self, licence):
        ...
        if portal_type in ['BuildLicence', 'Article127', 'UniqueLicence', 'IntegratedLicence']:
            if licence.AUTORISATION and datetime.strptime(licence.AUTORISATION, "%d/%m/%y 00:00:00") >= self.CODT_DATE and int(licence.AUTORISATION[6:8]) < 23:
                print("{} => CODT: {}".format(licence.NUM_PERMIS or licence.N1, licence.AUTORISATION))
                portal_type = "{}{}".format("CODT_", portal_type)
            elif licence.REFUS and datetime.strptime(licence.REFUS, "%d/%m/%y 00:00:00") >= self.CODT_DATE and int(licence.REFUS[6:8]) < 23:
